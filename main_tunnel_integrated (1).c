	...
	for (i = 0; i < c->qconf->nb_crypto_devs; i++) {
		if (c->port_cparams[i].do_cipher) {
			if (cs_rotate(&c->port_cparams[i].cs, key, epoch) == 0)
				/* keep the port's active session pointer current */
				c->port_cparams[i].session =
					cs_session_current(&c->port_cparams[i].cs, NULL);
		}
	}
	keyrot_stats.rotations++;          /* >>> INTEGRATION <<< */
	keyrot_stats.cur_epoch = epoch;    /* >>> INTEGRATION <<< */
}
	...
	while (1)
	{
		...
		if (unlikely(diff_tsc > drain_tsc))
		{
			 ...
			 * (stamped with the old epoch) still decrypt. <<< */
			{
				...
				kl_poll(kr_install_cb, kr_retire_cb, &kctx);
			}
			/* <<< INTEGRATION <<< */
					...
					if (portid == 1) {
						 ...
						 */
						 ...
						 if (tunnel_decap(m, &rx_epoch) < 0) {
							keyrot_stats.drop_decap++;   /* >>> INTEGRATION <<< */
							rte_crypto_op_free(ops_burst[j]);
							continue;
						}
						 ...
						 * stashed in mbuf headroom for use at dequeue time.
						 * NULL passed for pkt_iv (not used on encrypt path).
						 */
						/* >>> INTEGRATION: the epoch we STAMP must match the
						 * session we ENCRYPT with. Both come from cs_port, so the
						 * receiver's cs_session_for_epoch() will select the same
						 * key. (km_current_epoch is only the PSK-bootstrap epoch.) */
						uint8_t tx_epoch = cparams->cs.epoch_cur;
						keyrot_stats.pkts_encrypted++;        /* >>> INTEGRATION <<< */
						keyrot_stats.cur_epoch = tx_epoch;    /* >>> INTEGRATION <<< */
						l2fwd_simple_crypto_enqueue(m, ops_burst[j],
