...
/* >>> INTEGRATION: control-plane + key-rotation modules <<< */
#include "dbg.h"
#include "key_mgmt.h"        /* PSK bootstrap (epoch-0) */
...
#include "tunnel_frag.h"   /* >>> FRAG: fragmentation + reassembly module <<< */
...
#define SESSION_POOL_CACHE_SIZE 512
...
/* ── L2TP TUNNEL MODE ────────────────────────────────────────────────────────
 *
 * The COMPLETE original IP packet (inner IP header + transport header +
 * payload) is encrypted by CAAM.  Before TX a new outer IP header and a
 * UDP header are prepended so the wire carries:
 *
 *   [ETH 14B][Outer-IP 20B][UDP 8B][IV 16B][Encrypted(Inner-IP+TCP/UDP+Data)]
 *
 * Receive side (port 1):
 *   1. Strip outer IP (tunnel_decap)
 *   2. Strip outer UDP (8 bytes)
 *   3. Extract IV (16 bytes) → strip IV from mbuf
 *   4. CAAM decrypt → restores original [Inner-IP + transport + payload]
 *   5. Trim AES-CBC padding
 *   6. Forward to server
 *
 * ipdata_offset in l2fwd_simple_crypto_enqueue = sizeof(ETH) = 14
 * so CAAM encrypts/decrypts everything after the Ethernet header,
 * i.e. the full inner IP packet.
 * ─────────────────────────────────────────────────────────────────────────── */
#define TUNNEL_SRC_IP   RTE_IPV4(10, 0, 0, 1)
#define TUNNEL_DST_IP   RTE_IPV4(10, 0, 0, 2)
/*
 * TUNNEL_PROTO = 253 (RFC 3692 experimental/unassigned).
 * Using ESP(50) caused Wireshark to parse the payload as an ESP header
 * (showing bogus SPI/sequence fields) instead of showing a clean outer IP.
 * 253 is unassigned so Wireshark shows it as "Unknown" and displays the
 * outer IP header fields correctly without attempting inner dissection.
 * tunnel_decap verifies this value to confirm the packet is ours.
 */
#define TUNNEL_PROTO    253
#define TUNNEL_TTL      64
#define TUNNEL_UDP_SRC  1701   /* L2TP well-known port */
#define TUNNEL_UDP_DST  1701

/* Per-packet IV: 16 bytes placed after the outer UDP header, before the
 * encrypted payload.  Generated fresh for every packet using rte_rand()
 * (zero syscall overhead, safe at line rate).
 * Stashed in mbuf headroom during CAAM enqueue so the dequeue loop can
 * embed it in the wire packet without any shared-buffer race. */
#define IV_IN_PACKET_SIZE  16


struct iv_store {
    uint8_t iv[IV_IN_PACKET_SIZE];
};

static inline void
generate_per_pkt_iv(uint8_t *buf)
{
...
struct l2fwd_crypto_params
{
	...
	/* >>> INTEGRATION: per-epoch session table for key rotation <<< */
	struct cs_port cs;
	/* <<< INTEGRATION <<< */
	...
	uint8_t do_aead;
	uint8_t hash_verify;
	uint8_t is_encrypt;  /* 1 = port0 encrypt, 0 = port1 decrypt */
...
/* >>> INTEGRATION: key-rotation / data-path diagnostic counters.
 * These are incremented in the hot path but are plain uint64_t writes (no
 * per-packet printf), so they don't hurt throughput. They are printed only
 * in the periodic print_stats() block. Each lcore touches its own ports, so
 * for the typical one-lcore-per-port layout there is no real contention; if
 * you later share a port across lcores, make these per-lcore and sum them. */
struct l2fwd_keyrot_stats {
	uint64_t pkts_encrypted;     /* packets handed to encrypt path        */
	uint64_t pkts_decrypted;     /* packets successfully decap+decrypted   */
	uint64_t drop_bad_epoch;     /* decrypt: epoch had no matching session */
	uint64_t drop_decap;         /* tunnel_decap rejected the packet       */
	uint64_t rotations;          /* key rotations applied on this box      */
	uint8_t  cur_epoch;          /* last epoch seen/installed              */
};
static struct l2fwd_keyrot_stats keyrot_stats;

/* >>> CROSS-LCORE KEY SHARING:
 * kl_poll() is called by BOTH lcores every drain interval but only ONE
 * lcore will recv() the keyd message first -- the other gets nothing.
 * In a 2-lcore / 2-port setup:
 *   lcore=0 owns port 0 (encrypt) -- crypto dev 0
 *   lcore=1 owns port 1 (decrypt) -- crypto dev 1
 * Whichever lcore receives the key MUST share it with the other so BOTH
 * sessions rotate together. Without this, one lcore stays on epoch 0
 * forever while the other advances -- causing all packets to be dropped
 * because the epoch stamp no longer matches the decrypt session.
 *
 * g_shared_key:   the latest key received from keyd
 * g_shared_epoch: epoch number for that key (0xFF = none yet)
 * g_shared_seq:   incremented each time a new key is stored
 *                 each lcore tracks the last seq it applied
 *
 * Written by whichever lcore wins the recv() race.
 * Read by the other lcore on its next drain tick.
 * Plain writes are safe here: worst case the other lcore applies the
 * key one drain interval (100us) later, well within the 10s overlap. <<< */
static volatile uint8_t  g_shared_key[KM_KEY_LEN];
static volatile uint8_t  g_shared_epoch  = 0xFF;  /* 0xFF = not set yet */
static volatile uint32_t g_shared_seq    = 0;      /* increments each install */
/* <<< INTEGRATION <<< */
...
print_stats(void)
{
	...
	/* >>> INTEGRATION: key-rotation / data-path diagnostics <<< */
	printf("\nKey-rotation statistics ============================"
		   "\nPackets encrypted: %27" PRIu64
		   "\nPackets decrypted: %27" PRIu64
		   "\nDropped (unknown epoch): %21" PRIu64
		   "\nDropped (decap reject): %22" PRIu64
		   "\nKey rotations applied: %23" PRIu64
		   "\nCurrent epoch: %31u",
		   keyrot_stats.pkts_encrypted,
		   keyrot_stats.pkts_decrypted,
		   keyrot_stats.drop_bad_epoch,
		   keyrot_stats.drop_decap,
		   keyrot_stats.rotations,
		   (unsigned)keyrot_stats.cur_epoch);
	printf("\n====================================================\n");
	/* <<< INTEGRATION <<< */
...
l2fwd_simple_crypto_enqueue(struct rte_mbuf *m,
							...
							struct l2fwd_crypto_params *cparams,
							const uint8_t *pkt_iv,
							uint8_t epoch)   /* >>> INTEGRATION: key epoch <<< */
{
	struct rte_ether_hdr *eth_hdr __rte_unused;
	struct rte_ipv4_hdr *ip_hdr __rte_unused;
	...
	/*
	 * TUNNEL MODE: encrypt the full inner IP packet.
	 * ipdata_offset = ETH(14) — CAAM encrypts from inner IP onward.
	 */
	...
	/* >>> INTEGRATION: pick the session matching this packet's key epoch.
	 * Encrypt side passes km/keyd current epoch; decrypt side passes the
	 * epoch read from the packet (outer-IP TOS). Falls back to the static
	 * cparams->session only if the epoch table is empty (pre-first-key). <<< */
	{
		void *sess = cs_session_for_epoch(&cparams->cs, epoch);
		if (sess == NULL)
			sess = cparams->session;   /* bootstrap / fallback */
	...
	if (cparams->do_hash)
	{
		if (cparams->auth_iv.length)
		{
			uint8_t *iv_ptr = rte_crypto_op_ctod_offset(op, uint8_t *,
								IV_OFFSET + cparams->cipher_iv.length);
			rte_memcpy(iv_ptr, cparams->auth_iv.data, cparams->auth_iv.length);
		}
		if (!cparams->hash_verify)
			op->sym->auth.digest.data =
				(uint8_t *)rte_pktmbuf_append(m, cparams->digest_length);
		else
			op->sym->auth.digest.data =
				rte_pktmbuf_mtod(m, uint8_t *) + ipdata_offset + data_len;
	...
	if (cparams->do_cipher)
	{
		...
		/*
		 * Per-packet IV — burst-safe, no shared buffer:
		 *
		 * Encrypt (port 0, is_encrypt=1):
		 *   Generate fresh random IV → write to op slot (CAAM uses it).
		 *   Stash copy in mbuf headroom so the dequeue loop can embed
		 *   it in the wire packet after CAAM finishes.
		 *
		 * Decrypt (port 1, is_encrypt=0):
		 *   pkt_iv is a C stack-local array filled from the wire packet
		 *   in the RX loop — one array per loop iteration, never shared.
		 */
		// if (cparams->is_encrypt) {
		// 	generate_per_pkt_iv(iv_ptr);
		// 	uint8_t *stash = (uint8_t *)m->buf_addr
		// 					 + m->data_off - IV_IN_PACKET_SIZE;
		// 	rte_memcpy(stash, iv_ptr, IV_IN_PACKET_SIZE);
		// } else {
		// 	rte_memcpy(iv_ptr, pkt_iv, cparams->cipher_iv.length);
		// }
		...
		if (cparams->is_encrypt)
		{
			...
			* Encrypt path:
			*
			* Generate fresh IV for this packet.
			* CAAM uses iv_ptr directly.
			*/
			...
			/*
			* Save a copy of IV inside crypto-op private area.
			*
			* DO NOT use mbuf headroom.
			* Headroom gets corrupted after prepend/adjust.
			*/
		...
		else
		{
			/*
			* Decrypt path:
			* pkt_iv extracted from wire packet.
			*/
	...
	if (cparams->do_aead)
	{
		...
		if (cparams->aead_algo == RTE_CRYPTO_AEAD_AES_CCM)
			rte_memcpy(iv_ptr + 1, cparams->aead_iv.data, cparams->aead_iv.length);
		else
...
/*
 * tunnel_encap_tunnel_mode
 *
 * Prepends [Outer-IP 20B][UDP 8B][IV 16B] in one shot after the ETH header.
 *
 * Wire layout after this function:
 *   [ETH 14B][Outer-IP 20B][UDP 8B][IV 16B][Encrypted-Inner-IP + Transport + Data]
 *
 * Outer-IP proto = TUNNEL_PROTO (ESP/50) so tunnel_decap can verify it.
 * Outer-IP total_length covers: outer-IP + UDP + IV + encrypted inner.
 * UDP length covers: UDP hdr + IV + encrypted inner.
 */
// static int
// tunnel_encap(struct rte_mbuf *m, uint16_t src_portid __rte_unused,
...
//     /* inner_len = everything after ETH (encrypted inner IP packet) */
//     inner_len = (uint16_t)(m->pkt_len - sizeof(struct rte_ether_hdr));

//     rte_memcpy(&eth_copy,
//                rte_pktmbuf_mtod(m, struct rte_ether_hdr *),
//                sizeof(struct rte_ether_hdr));

//     /* Prepend space for Outer-IP + UDP + IV in one call */
//     uint16_t prepend = (uint16_t)(sizeof(struct rte_ipv4_hdr)
//                                   + sizeof(struct rte_udp_hdr)
//                                   + IV_IN_PACKET_SIZE);
//     if (rte_pktmbuf_prepend(m, prepend) == NULL)
//         return -1;

//     /* Restore ETH at new front */
//     rte_memcpy(rte_pktmbuf_mtod(m, void *), &eth_copy,
//                sizeof(struct rte_ether_hdr));

//     /* Outer IP header */
//     outer_ip = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(m, char *)
//                + sizeof(struct rte_ether_hdr));
...
//     outer_ip->fragment_offset = rte_cpu_to_be_16(RTE_IPV4_HDR_DF_FLAG);
//     outer_ip->time_to_live    = TUNNEL_TTL;
//     outer_ip->next_proto_id   = TUNNEL_PROTO;   /* marks this as our tunnel */
//     outer_ip->hdr_checksum    = 0;
//     outer_ip->src_addr        = rte_cpu_to_be_32(TUNNEL_SRC_IP);
//     outer_ip->dst_addr        = rte_cpu_to_be_32(TUNNEL_DST_IP);
//     outer_ip->hdr_checksum    = rte_ipv4_cksum(outer_ip);

//     /* Outer UDP header — L2TP conventional ports */
//     outer_udp = (struct rte_udp_hdr *)((char *)outer_ip
//                 + sizeof(struct rte_ipv4_hdr));
//     outer_udp->src_port   = rte_cpu_to_be_16(TUNNEL_UDP_SRC);
//     outer_udp->dst_port   = rte_cpu_to_be_16(TUNNEL_UDP_DST);
//     outer_udp->dgram_len  = rte_cpu_to_be_16(
//                                 (uint16_t)(sizeof(struct rte_udp_hdr)
//                                            + IV_IN_PACKET_SIZE
//                                            + inner_len));
//     outer_udp->dgram_cksum = 0;  /* checksum disabled for performance */

//     /* IV immediately after UDP header */
//     rte_memcpy((char *)outer_udp + sizeof(struct rte_udp_hdr),
//                iv_data, IV_IN_PACKET_SIZE);
...
static int
tunnel_encap(struct rte_mbuf *m,
             uint16_t src_portid __rte_unused,
             const uint8_t *iv_data,
             uint8_t epoch)        /* >>> INTEGRATION: key epoch <<< */
{
    struct rte_ether_hdr eth_copy;
    ...
    struct rte_udp_hdr *outer_udp;
    uint16_t inner_len;
    uint16_t prepend_len;
    char *new_ptr;

    /*
     * Current packet before encap:
     *
     * [ETH][Encrypted Inner-IP + Transport + Payload + Padding + Digest]
     */
    ...
    /*
     * Remove Ethernet header temporarily
     */
    if (rte_pktmbuf_adj(m,
    ...
    /*
     * Prepend required space
     */
    new_ptr = rte_pktmbuf_prepend(m, prepend_len);
     ...
     * Final packet layout:
     *
     * [ETH][Outer-IP][UDP][IV][Encrypted Payload]
     */

    /* Restore Ethernet header */
    eth_hdr = (struct rte_ether_hdr *)new_ptr;
     ...
     * Outer IPv4 header
     */
    outer_ip = (struct rte_ipv4_hdr *)
        (new_ptr + sizeof(struct rte_ether_hdr));

    outer_ip->version_ihl = RTE_IPV4_VHL_DEF;
    outer_ip->type_of_service = epoch;   /* >>> INTEGRATION: carry key epoch <<< */
    ...
    outer_ip->src_addr =
        rte_cpu_to_be_32(TUNNEL_SRC_IP);

    outer_ip->dst_addr =
        rte_cpu_to_be_32(TUNNEL_DST_IP);

    outer_ip->hdr_checksum = 0;

    outer_ip->hdr_checksum =
        rte_ipv4_cksum(outer_ip);

    /*
     * UDP header
     */
    outer_udp = (struct rte_udp_hdr *)
        ((char *)outer_ip +
         sizeof(struct rte_ipv4_hdr));

    outer_udp->src_port =
        rte_cpu_to_be_16(TUNNEL_UDP_SRC);

    outer_udp->dst_port =
        rte_cpu_to_be_16(TUNNEL_UDP_DST);
...
/*
 * tunnel_decap_tunnel_mode
 *
 * Strips [Outer-IP 20B][UDP 8B] from the packet.
 * After this the layout is:
 *   [ETH 14B][IV 16B][Encrypted-Inner-IP + Transport + Data]
 *
 * Returns the outer IP's total_length field (host byte order) so the
 * caller can trim AES-CBC padding after decryption.
 */
// static int
// tunnel_decap(struct rte_mbuf *m)
// {
//     struct rte_ether_hdr  eth_copy;
//     struct rte_ether_hdr *eth_hdr;
//     struct rte_ipv4_hdr  *outer_ip;
...
//     eth_hdr  = rte_pktmbuf_mtod(m, struct rte_ether_hdr *);
//     outer_ip = (struct rte_ipv4_hdr *)(eth_hdr + 1);

//     if (outer_ip->next_proto_id != TUNNEL_PROTO)
//         return -1;

//     rte_memcpy(&eth_copy, eth_hdr, sizeof(struct rte_ether_hdr));

//     /* Strip outer IP + outer UDP in one adj */
//     uint16_t strip = (uint16_t)(sizeof(struct rte_ipv4_hdr)
//                                 + sizeof(struct rte_udp_hdr));
//     if (rte_pktmbuf_adj(m, strip) == NULL)
//         return -1;
...
// }
static int
tunnel_decap(struct rte_mbuf *m, uint8_t *epoch_out)   /* >>> INTEGRATION: epoch <<< */
{
    struct rte_ether_hdr eth_copy;
    struct rte_ipv4_hdr *outer_ip;
    uint16_t strip;

    /* Verify this is our UDP tunnel packet */
    outer_ip = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(m, char *)
               + sizeof(struct rte_ether_hdr));
    if (unlikely(outer_ip->next_proto_id != IPPROTO_UDP))
        return -1;

    /* >>> INTEGRATION: recover the key epoch carried in the outer-IP TOS
     * byte (written by tunnel_encap on the peer). Read it BEFORE stripping. <<< */
    if (epoch_out)
        *epoch_out = outer_ip->type_of_service;

    /* Save ETH header before stripping */
    rte_memcpy(&eth_copy, rte_pktmbuf_mtod(m, void *),
               sizeof(struct rte_ether_hdr));

    /*
     * Strip [ETH + Outer-IP + UDP] in a single adj — no memmove, no trim,
     * just a data_off pointer bump.  After adj the front of the mbuf is:
     *   [IV 16B][Encrypted Inner-IP + Transport + Data]
     */
    strip = (uint16_t)(sizeof(struct rte_ether_hdr)
                       + sizeof(struct rte_ipv4_hdr)
                       + sizeof(struct rte_udp_hdr));
    if (unlikely(rte_pktmbuf_adj(m, strip) == NULL))
        return -1;

    /* Carve fresh ETH space from headroom and restore the saved header */
    if (unlikely(rte_pktmbuf_prepend(m, sizeof(struct rte_ether_hdr)) == NULL))
        return -1;
...
l2fwd_simple_forward(struct rte_mbuf *m, uint16_t portid,
                     ...
                     const uint8_t *iv)
{
    uint16_t dst_port = l2fwd_dst_ports[portid];
    ...
    if (portid == 0) {
        /*
         * Encrypt side: prepend [Outer-IP][UDP][IV] then TX out port1.
         * iv is read from mbuf headroom (per-packet, burst-safe).
         */
		...
		/* >>> INTEGRATION: tunnel_encap now needs the key epoch to stamp into
		 * the outer-IP TOS. It MUST equal the epoch whose session encrypted
		 * this packet at enqueue time (tx_epoch = cparams->cs.epoch_cur there).
		 *
		 * Because l2fwd_simple_forward runs at DEQUEUE time and does not
		 * currently receive cparams, choose ONE of:
		 *   (a) Simplest: pass the lcore's current encrypt epoch here. Since
		 *       rotation is every 120s and the overlap window accepts the
		 *       previous epoch, a rare enqueue/dequeue straddle is tolerated.
		 *       Requires making the current encrypt cs_port reachable here
		 *       (e.g. a per-lcore pointer set at init), then:
		 *           uint8_t enc_epoch = g_lcore_enc_cs->epoch_cur;
		 *   (b) Exact: stash tx_epoch into mbuf headroom right next to the IV
		 *       at enqueue, and read it back here (mirrors how `iv` is carried).
		 *
		 * Until wired, this passes 0 so the file COMPILES and single-epoch
        ...
        /*
         * Decrypt side: CAAM has restored [Inner-IP + Transport + Data].
         * Trim auth digest if any, then trim AES-CBC padding using the
         * inner IP's total_length field.
         *
         * Layout at this point:
         *   [ETH 14B][Inner-IP 20B][Transport hdr][Decrypted Data][Padding]
         */
        if (options->auth_xform.auth.digest_length > 0)
            rte_pktmbuf_trim(m, options->auth_xform.auth.digest_length);

        inner_ip = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(m, char *)
                   + eth_len);
        pad_len = m->pkt_len
                  - eth_len
                  - rte_be_to_cpu_16(inner_ip->total_length);
...
/* >>> INTEGRATION: key-rotation callbacks for kl_poll() <<<
 * When keyd pushes a new key, rebuild the per-epoch session on every crypto
 * device this lcore owns. Each lcore runs its own kl_poll, so each rebuilds
 * only its own port's session — no cross-lcore session sharing. */
struct kr_ctx {
	struct lcore_queue_conf  *qconf;
	struct l2fwd_crypto_params *port_cparams;
};

static void kr_install_cb(uint8_t epoch, const uint8_t *key, void *arg)
{
	struct kr_ctx *c = (struct kr_ctx *)arg;
	unsigned i;

	printf("[KEY] kr_install_cb CALLED: epoch=%u nb_crypto_devs=%u lcore=%u\n",
	       epoch, c->qconf->nb_crypto_devs, rte_lcore_id());
	fflush(stdout);

	/* >>> CROSS-LCORE: store key globally so the OTHER lcore can also
	 * rotate its session on its next drain tick. This is the fix for
	 * the bug where only one lcore rotates and ping stops after keyd
	 * installs the first TLS-derived key. <<< */
	memcpy((void *)g_shared_key, key, KM_KEY_LEN);
	g_shared_epoch = epoch;
	rte_wmb();             /* ensure key bytes written before seq bump */
	g_shared_seq++;
	printf("[KEY] stored in g_shared: epoch=%u seq=%u\n",
	       epoch, g_shared_seq);
	fflush(stdout);

	/* Rotate every crypto device this lcore owns */
	for (i = 0; i < c->qconf->nb_crypto_devs; i++) {
		if (c->port_cparams[i].do_cipher) {
			printf("[KEY] rotating dev=%u is_encrypt=%u cur_epoch=%u -> %u\n",
			       c->port_cparams[i].dev_id,
			       c->port_cparams[i].is_encrypt,
			       c->port_cparams[i].cs.epoch_cur,
			       epoch);
			fflush(stdout);

			if (cs_rotate(&c->port_cparams[i].cs, key, epoch) == 0) {
				c->port_cparams[i].session =
					cs_session_current(&c->port_cparams[i].cs, NULL);
				printf("[KEY] cs_rotate OK dev=%u cur=%u prev=%u have_prev=%d\n",
				       c->port_cparams[i].dev_id,
				       c->port_cparams[i].cs.epoch_cur,
				       c->port_cparams[i].cs.epoch_prev,
				       c->port_cparams[i].cs.have_prev);
			} else {
				printf("[KEY] cs_rotate FAILED dev=%u\n",
				       c->port_cparams[i].dev_id);
			}
			fflush(stdout);
		}
	...
	keyrot_stats.rotations++;
	keyrot_stats.cur_epoch = epoch;
	printf("[KEY] install complete lcore=%u rotations=%lu cur_epoch=%u\n",
	       rte_lcore_id(),
	       (unsigned long)keyrot_stats.rotations,
	       (unsigned)keyrot_stats.cur_epoch);
	fflush(stdout);
}

static void kr_retire_cb(uint8_t epoch, void *arg)
{
	struct kr_ctx *c = (struct kr_ctx *)arg;
	unsigned i;
	(void)epoch;
	for (i = 0; i < c->qconf->nb_crypto_devs; i++)
		if (c->port_cparams[i].do_cipher)
			cs_retire_prev(&c->port_cparams[i].cs);
}
/* <<< INTEGRATION <<< */
...
l2fwd_main_loop(struct l2fwd_crypto_options *options)
{
	...
	/* >>> CROSS-LCORE: track last shared_seq this lcore applied so we
	 * know when a new key from the other lcore needs to be picked up. <<< */
	uint32_t lcore_applied_seq = 0;
	...
	/* Set encryption or decryption mode based on port */
	if (portid == 0)
	{
		// printf("Assigned ENCRYPT: port %u : \n", portid);
		options->cipher_xform.cipher.op = RTE_CRYPTO_CIPHER_OP_ENCRYPT;
		options->auth_xform.auth.op = RTE_CRYPTO_AUTH_OP_GENERATE;
	}
	else if (portid == 1)
	{
		// printf("Assigned DECRYPT: port %u \n", portid);
	...
	for (i = 0; i < qconf->nb_crypto_devs; i++)
	{
		...
		port_cparams[i].is_encrypt = (portid == 0) ? 1 : 0;
		...
		/* >>> INTEGRATION: build the epoch-0 entry of the per-epoch session
		 * table. Bootstrap key = the PSK loaded by km_init() (so the link
		 * works before the first keyd/TLS key arrives). The cipher xform was
		 * just configured above; cs_port_init builds its own session from a
		 * COPY of that xform with the bootstrap key. For cipher-only pass NULL
		 * for the auth xform; if you run a CIPHER_HASH/HASH_CIPHER chain, pass
		 * &options->auth_xform and confirm chain order in crypto_session.c. */
		if (port_cparams[i].do_cipher) {
			uint8_t k0[KM_KEY_LEN];
			uint8_t e0 = 0;
			if (km_current_key(k0, &e0) != KM_OK)
				rte_exit(EXIT_FAILURE,
				    "km_current_key failed (PSK bootstrap not initialised)\n");

			/* Resolve the session mempool for this device's NUMA socket,
			 * matching what initialize_crypto_session() uses. */
			int csid = rte_cryptodev_socket_id(port_cparams[i].dev_id);
			if (csid < 0)
				rte_exit(EXIT_FAILURE, "bad cryptodev socket id\n");
			struct rte_mempool *sess_mp =
				session_pool_socket[(uint8_t)csid].sess_mp;

			if (cs_port_init(&port_cparams[i].cs,
							 port_cparams[i].dev_id,
							 port_cparams[i].is_encrypt,
							 &options->cipher_xform,
							 NULL,
							 sess_mp,
							 k0, e0) != 0)
				rte_exit(EXIT_FAILURE, "cs_port_init failed\n");

			explicit_bzero(k0, sizeof(k0));
	...
	while (1)
	{
		...
		{
			/* >>> INTEGRATION: pick up any new key pushed by keyd and rotate
			 * this lcore's per-port session(s). kl_poll is non-blocking and
			 * cheap; on a key install it calls kr_install_cb (below), which
			 * cs_rotate()s every crypto dev on this lcore. The previous-epoch
			 * session is kept for the overlap window so in-flight packets
			 * (stamped with the old epoch) still decrypt. <<< */
			/* >>> INTEGRATION + CROSS-LCORE KEY FIX:
			 * Step A: kl_poll() — whichever lcore wins the recv()
			 *   race gets the key, stores it in g_shared_key, and
			 *   rotates its own session via kr_install_cb.
			 * Step B: every lcore checks g_shared_seq each drain.
			 *   If it advanced since last time this lcore applied it,
			 *   the OTHER lcore got a new key — apply it here too.
			 *   This ensures BOTH lcores rotate together so the epoch
			 *   stamp on encrypted packets always matches the decrypt
			 *   session on the peer box. <<< */
			{
				struct kr_ctx kctx = { .qconf = qconf,
									   .port_cparams = port_cparams };

				/* Step A: try to receive key from keyd */
				int kl_ret = kl_poll(kr_install_cb, kr_retire_cb, &kctx);
				if (kl_ret > 0) {
					lcore_applied_seq = g_shared_seq;
					printf("[KL] lcore=%u received %d msg(s) from keyd\n",
					       lcore_id, kl_ret);
					fflush(stdout);
				}

				/* Step B: did the OTHER lcore get a new key? */
				rte_rmb();
				if (g_shared_epoch != 0xFF &&
				    g_shared_seq != lcore_applied_seq) {
					printf("[KEY] lcore=%u applying cross-lcore key:"
					       " epoch=%u seq=%u (was seq=%u)\n",
					       lcore_id, g_shared_epoch,
					       g_shared_seq, lcore_applied_seq);
					fflush(stdout);

					/* Apply the shared key to THIS lcore's devices */
					uint8_t tmp_key[KM_KEY_LEN];
					uint8_t tmp_epoch = g_shared_epoch;
					memcpy(tmp_key, (void *)g_shared_key, KM_KEY_LEN);
					lcore_applied_seq = g_shared_seq;

					unsigned ci;
					for (ci = 0; ci < qconf->nb_crypto_devs; ci++) {
						if (port_cparams[ci].do_cipher) {
							printf("[KEY] cross-lcore rotate dev=%u"
							       " is_encrypt=%u cur=%u -> %u\n",
							       port_cparams[ci].dev_id,
							       port_cparams[ci].is_encrypt,
							       port_cparams[ci].cs.epoch_cur,
							       tmp_epoch);
							fflush(stdout);
							if (cs_rotate(&port_cparams[ci].cs,
							              tmp_key, tmp_epoch) == 0) {
								port_cparams[ci].session =
									cs_session_current(
									    &port_cparams[ci].cs, NULL);
								/* update enc_cs pointer if encrypt lcore */
								if (port_cparams[ci].is_encrypt)
									RTE_PER_LCORE(g_enc_cs) =
										&port_cparams[ci].cs;
								printf("[KEY] cross-lcore OK dev=%u"
								       " cur=%u prev=%u\n",
								       port_cparams[ci].dev_id,
								       port_cparams[ci].cs.epoch_cur,
								       port_cparams[ci].cs.epoch_prev);
							} else {
								printf("[KEY] cross-lcore FAILED dev=%u\n",
								       port_cparams[ci].dev_id);
							}
							fflush(stdout);
						}
					}
					keyrot_stats.cur_epoch = tmp_epoch;
					explicit_bzero(tmp_key, sizeof(tmp_key));
				}
			}
			/* <<< INTEGRATION <<< */

			/* >>> FRAG: evict stale incomplete fragment sequences from both
			 * reassembly tables (RX inbound + decap). Without this periodic
			 * call, a single lost fragment permanently occupies a table slot
			 * until the table fills and new flows are silently dropped.
			 * This is the fix for the class of slow-resource-exhaustion bug
			 * that caused the vendor's 3-day failure mode. Cheap when idle. <<< */
			tf_drain_death_row();
			/* <<< FRAG <<< */
		...
		for (i = 0; i < qconf->nb_rx_ports; i++) {
			...
			//dst_port = rte_be_to_cpu_16(udp_hdr->dst_port);
				...
				//if (dst_port == 5020) {
				//printf("The value of nb_rx is %d\n\n", nb_rx);
			 	//}
		        //}	
			...
			if (nb_rx) {
				...
				/* Debug print removed — was overwriting ether_type incorrectly */
				 ...
				 *
				 * ARP → forward directly (no crypto, so ARP exchange works
				 *        between Client and Kit1, or Server and Kit2)
				 *
				 * Port1 non-ARP → strip tunnel outer IP before crypto
				 *   (tunnel_decap removes the outer IP that Kit1 added)
				 *   then pass to CAAM for decryption
				 *
				 * Port0 non-ARP → pass straight to CAAM for encryption
				 *   tunnel_encap is called AFTER CAAM returns in l2fwd_simple_forward
				 */
				for (j = 0; j < nb_rx; j++) {
					m = pkts_burst[j];
					...
					/* ARP: forward without crypto, free the pre-allocated op */
					if (pkt_eth->ether_type ==
					...
					if (portid == 1) {
						/*
						 * TUNNEL MODE DECRYPT path:
						 *
						 * Received wire layout:
						 *   [ETH][Outer-IP][UDP][IV 16B][Encrypted-Inner-IP+Transport+Data]
						 *
						 * Step 1: tunnel_decap strips [Outer-IP + UDP] (28B total).
						 * After decap:
						 *   [ETH][IV 16B][Encrypted-Inner-IP+Transport+Data]
						 *
						 * Step 2: Extract IV into stack-local array (burst-safe).
						 * Step 3: Strip IV from mbuf.
						 * After strip:
						 *   [ETH][Encrypted-Inner-IP+Transport+Data]
						 *
						 * Step 4: enqueue to CAAM with the extracted IV.
						 */
						 ...
						 uint8_t rx_epoch = 0;   /* >>> INTEGRATION <<< */
						 if (tunnel_decap(m, &rx_epoch) < 0) {
							keyrot_stats.drop_decap++;
							printf("[DROP] tunnel_decap FAILED pkt_len=%u
", m->pkt_len);
							fflush(stdout);
							rte_crypto_op_free(ops_burst[j]);
							continue;
						}
						/* >>> INTEGRATION: verify we hold a session for this
						 * packet's epoch BEFORE enqueue; if not, drop and count
						 * (rotation overlap should normally prevent this). */
						if (cs_session_for_epoch(&cparams->cs, rx_epoch) == NULL
								&& cparams->cs.epoch_cur != rx_epoch) {
							printf("[DROP] bad epoch=%u cur=%u prev=%u have_prev=%d
",
							       rx_epoch, cparams->cs.epoch_cur,
							       cparams->cs.epoch_prev, cparams->cs.have_prev);
							fflush(stdout);
							keyrot_stats.drop_bad_epoch++;
							keyrot_stats.cur_epoch = rx_epoch;
							rte_pktmbuf_free(m);
							rte_crypto_op_free(ops_burst[j]);
							continue;
						}
						...
						/* Extract IV — stack-local, one per packet, never shared */
						uint8_t pkt_iv[IV_IN_PACKET_SIZE];
						uint8_t *pd = rte_pktmbuf_mtod(m, uint8_t *);
						rte_memcpy(pkt_iv,
						...
						rte_memcpy(rte_pktmbuf_mtod(m, void *),
								   &eth_tmp, sizeof(struct rte_ether_hdr));

						l2fwd_simple_crypto_enqueue(m, ops_burst[j],
								cparams, pkt_iv, rx_epoch);   /* >>> INTEGRATION <<< */
					} else {
						/*
						 * TUNNEL MODE ENCRYPT path (port 0):
						 *
						 * Packet layout:
						 *   [ETH][Inner-IP][Transport][Data]
						 *
						 * CAAM encrypts from ETH+14 onward (full inner IP packet).
						 * IV is generated inside l2fwd_simple_crypto_enqueue and
						 * stashed in crypto-op private area for use at dequeue.
						 * NULL passed for pkt_iv (not used on encrypt path).
						 */

						/* >>> FRAG SCENARIO 2: reassemble inbound client IP
						 * fragments BEFORE encryption. If the client sent a
						 * fragmented packet (e.g. jumbo frame broken upstream,
						 * or PMTUD failure), we must reconstruct the complete
						 * inner packet before encrypting -- otherwise each
						 * fragment gets its own tunnel header and the peer
						 * cannot reassemble after decryption.
						 * tf_reassemble_rx() returns NULL while waiting for
						 * more fragments (mbuf is owned by table -- do NOT
						 * free or enqueue it). Returns the original mbuf
						 * immediately if it is NOT a fragment (fast path). <<< */
						m = tf_reassemble_rx(m, cur_tsc);
						if (m == NULL) {
							/* Still waiting for more fragments.
							 * Free the pre-allocated crypto op but do NOT
							 * free m -- it belongs to the reassembly table. */
							rte_crypto_op_free(ops_burst[j]);
							continue;
						}

						/* >>> FRAG SCENARIO 1 TX: check if the inner packet
						 * is too large to fit in the tunnel after tunnel_encap()
						 * adds TF_TUNNEL_OVERHEAD (58B) of outer headers.
						 * If so, tf_fragment_inner() splits it into N smaller
						 * fragments. Each fragment is individually encrypted
						 * and encapsulated. The peer decrypts each one, strips
						 * the tunnel header, then reassembles (see SCENARIO 1
						 * RX below in the dequeue path).
						 *
						 * CRITICAL: each fragment gets its own UNIQUE IV via
						 * generate_per_pkt_iv() inside l2fwd_simple_crypto_enqueue
						 * -- called separately for each fragment below. IV reuse
						 * across fragments under the same key is a cryptographic
						 * weakness for AES-CBC mode. <<< */
						{
							struct rte_mbuf *frags[TF_MAX_FRAG_NUM];
							int nfrags = tf_fragment_inner(m, frags);

							if (nfrags == 0) {
								/* Fragmentation failed, packet already freed
								 * inside tf_fragment_inner(). Free op only. */
								rte_crypto_op_free(ops_burst[j]);
								continue;
							}

							uint8_t tx_epoch = cparams->cs.epoch_cur;
							keyrot_stats.pkts_encrypted += nfrags;
							keyrot_stats.cur_epoch = tx_epoch;

							if (nfrags == 1) {
								/* Common case: no split needed. Enqueue the
								 * original (or reassembled) packet directly. */
								l2fwd_simple_crypto_enqueue(frags[0],
										ops_burst[j],
										cparams, NULL, tx_epoch);
							} else {
								/* Packet was split. Allocate a fresh crypto op
								 * for each fragment beyond the first, then
								 * enqueue all independently. The pre-allocated
								 * ops_burst[j] is used for fragment 0. */
								int fi;
								for (fi = 0; fi < nfrags; fi++) {
									struct rte_crypto_op *fop;
									if (fi == 0) {
										fop = ops_burst[j];
									} else {
										/* Allocate a new op for this fragment.
										 * If alloc fails, drop this fragment
										 * only -- the others still get through. */
										if (rte_crypto_op_bulk_alloc(
												l2fwd_crypto_op_pool,
												RTE_CRYPTO_OP_TYPE_SYMMETRIC,
												&fop, 1) != 1) {
											rte_pktmbuf_free(frags[fi]);
											continue;
										}
									}
									l2fwd_simple_crypto_enqueue(frags[fi],
											fop, cparams, NULL, tx_epoch);
								}
							}
						}
						/* <<< FRAG <<< */
				...
				/* Dequeue completed crypto ops from CAAM */
// 			do
// 			{
...
// 					/*
// 					 * Encrypt side (port 0):
// 					 *   Read IV stashed in mbuf headroom — per-packet,
// 					 *   burst-safe.  Pass to tunnel_encap which prepends
// 					 *   [Outer-IP][UDP][IV] in one shot.
// 					 *
// 					 * Decrypt side (port 1):
// 					 *   Trim padding, forward to server.
// 					 */
// 					const uint8_t *deq_iv = NULL;
// 					if (portid == 0 && cparams->do_cipher)
// 						deq_iv = (const uint8_t *)m->buf_addr
// 								 + m->data_off - IV_IN_PACKET_SIZE;

// 					l2fwd_simple_forward(m, portid, options, deq_iv);
// 				}
...
// }
			...
			do
			{
				...
				for (j = 0; j < nb_rx; j++)
				{
					...
					/*
					* Encrypt side:
					* Retrieve IV saved inside crypto-op memory.
					*/
					const uint8_t *deq_iv = NULL;
					...
					/*
					* tunnel_encap uses deq_iv here
					*/

					/* >>> FRAG SCENARIO 1 RX: on the decrypt side (port 1),
					 * the peer may have split an oversized inner packet into
					 * fragments before encrypting. Each fragment arrived
					 * encrypted independently, was decrypted individually
					 * above by CAAM, and the tunnel header was stripped by
					 * tunnel_decap() in l2fwd_simple_forward's port-1 branch.
					 *
					 * Wait -- actually l2fwd_simple_forward calls tunnel_decap
					 * BEFORE this dequeue point (tunnel_decap runs in the RX
					 * path, not here). At THIS point m is already decrypted
					 * and the outer header is already stripped. So we feed
					 * each decrypted inner fragment into the decap reassembly
					 * table here, BEFORE passing to l2fwd_simple_forward for
					 * forwarding to the server.
					 *
					 * tf_reassemble_decap() returns NULL while waiting for
					 * remaining fragments -- in that case do NOT forward.
					 * Returns the complete reassembled packet once all
					 * fragments have arrived. Non-fragments pass through
					 * immediately at zero table-lookup cost (fast path). <<< */
					if (portid == 1) {
						m = tf_reassemble_decap(m, cur_tsc);
						if (m == NULL) {
							/* Fragment buffered, waiting for rest.
							 * Op already freed below -- just skip forward. */
							rte_crypto_op_free(ops_burst[j]);
							continue;
						}
					}
	...
	if (options->xform_chain == L2FWD_CRYPTO_AEAD)
	{
		...
		if (options->aad_param)
		{
			if (check_supported_size(options->aad.length,
									 ...
									 cap->sym.aead.aad_size.increment) != 0)
			{
				RTE_LOG(DEBUG, USER1,
						"Device %u does not support "
						"AAD length\n",
		...
		options->xform_chain == L2FWD_CRYPTO_CIPHER_ONLY)
	{
		...
		if (options->ckey_param)
		{
			if (check_supported_size(
					...
					cap->sym.cipher.key_size.increment) != 0)
			{
		...
		options->xform_chain == L2FWD_CRYPTO_HASH_ONLY)
	{
		...
		if (options->akey_param)
		{
			if (check_supported_size(
					...
					cap->sym.auth.key_size.increment) != 0)
			{
				RTE_LOG(DEBUG, USER1,
		...
		if (options->digest_size != -1)
		{
			if (check_supported_size(options->digest_size,
									 ...
									 cap->sym.auth.digest_size.increment) != 0)
			{
				RTE_LOG(DEBUG, USER1,
						"Device %u does not support "
						"digest length\n",
					  ...
					  uint8_t *enabled_cdevs)
{
	...
	for (cdev_id = 0; cdev_id < cdev_count; cdev_id++)
	{
		...
		/* >>> INTEGRATION: key rotation keeps up to 2 sessions live per device
		 * (current + previous epoch during the overlap window), plus the
		 * original static session, plus headroom for the brief moment a rotate
		 * allocates the new one before the old is freed. Multiply the pool so
		 * cs_port_init() and cs_rotate() can always get an object. <<< */
		sessions_needed *= 4;
		/* <<< INTEGRATION <<< */
...
int main(int argc, char **argv)
{
	...
	/* >>> INTEGRATION: control-plane bootstrap (runs once, on main lcore) <<<
	 * 1. Debug mask: no effect unless built with -DL2FWD_DEBUG. Enable the
	 *    key subsystem so rotation is visible during bring-up. */
	dbg_set_mask(DBG_KEY | DBG_INIT);

	/* 2. Load the PSK and derive the epoch-0 bootstrap key. The data link uses
	 *    this until keyd delivers the first TLS-derived key. The same
	 *    /etc/l2fwd/psk.key (16 raw bytes, mode 600) must exist on both boxes. */
	if (km_init(0 /*=120s rotate, unused once keyd takes over*/,
				0 /*=10s overlap*/) != KM_OK)
		rte_exit(EXIT_FAILURE,
			"key bootstrap failed: is /etc/l2fwd/psk.key present, "
			"16 bytes, mode 600?\n");

	/* 3. Connect to keyd's AF_UNIX socket. Non-fatal if keyd isn't up yet;
	 *    kl_poll() in the data loop keeps retrying and installs keys as they
	...
	/* >>> FRAG: initialise fragmentation + reassembly tables. Fatal on failure
	 * because without this, oversized packets are silently dropped on the WAN
	 * path. Two separate tables are created: one for inbound client fragments
	 * (port 0 RX, before encryption) and one for decap'd inner fragments
	 * (port 1 RX, after decrypt + tunnel_decap). rte_socket_id() ensures
	 * NUMA-local allocation matching the other pools. <<< */
	if (tf_init(rte_socket_id()) != 0)
		rte_exit(EXIT_FAILURE,
			"tunnel_frag init failed -- check hugepage memory availability\n");
	/* <<< FRAG <<< */
