...
/* ── L2TP TRANSPORT MODE ─────────────────────────────────────────────────────
 *
 * The original IP header is kept INTACT and visible on the wire.
 * Only the transport layer (TCP/UDP/ICMP etc.) and its payload are encrypted.
 *
 * Before encryption, the original IP protocol field is saved, the IP proto
 * field is overwritten with UDP (17), and a UDP header + 2-byte proto
 * metadata field + 16-byte IV are inserted before the encrypted payload.
 *
 * Wire layout (TX from encrypt kit):
 *   [ETH 14B][Original-IP 20B][UDP 8B][ProtoMeta 2B][IV 16B]
 *   [Encrypted(Original-Transport-Hdr + Payload)]
 *
 * Receive side (port 1):
 *   1. Parse original IP — verify proto == UDP (our marker)
 *   2. Extract ProtoMeta (2B) → saved original protocol
 *   3. Extract IV (16B) → strip [UDP + ProtoMeta + IV] from mbuf
 *   4. Restore original IP proto field
 *   5. CAAM decrypt → restores [Transport-Hdr + Payload]
 *   6. Trim AES-CBC padding using original IP total_length
 *   7. Forward to server
 *
 * ipdata_offset in l2fwd_simple_crypto_enqueue:
 *   ETH(14) + IP(20) + UDP(8) + ProtoMeta(2) + IV(16) = 60
 *   so CAAM encrypts/decrypts only from byte 60 onward.
 * ─────────────────────────────────────────────────────────────────────────── */
#define TUNNEL_SRC_IP   RTE_IPV4(10, 0, 0, 1)
#define TUNNEL_DST_IP   RTE_IPV4(10, 0, 0, 2)
#define TUNNEL_PROTO    IPPROTO_ESP   /* used only as sentinel — not inserted */
#define TUNNEL_TTL      64
#define TRANSPORT_UDP_SRC  1701
#define TRANSPORT_UDP_DST  1701
#define TRANSPORT_PROTO_META_LEN  2   /* 2 bytes storing original IP proto */

/* Per-packet IV: 16 bytes placed after the ProtoMeta field.
 * Generated with rte_rand() — zero syscall, line-rate safe.
 * Stashed in mbuf headroom so the dequeue loop can embed it without
 * any shared-buffer race across burst packets. */
...
/* Total header overhead inserted before encrypted payload */
#define TRANSPORT_OVERHEAD  (sizeof(struct rte_udp_hdr) \
                             + TRANSPORT_PROTO_META_LEN \
                             + IV_IN_PACKET_SIZE)
							...
							const uint8_t *pkt_iv)
{
	...
	/*
	 * TRANSPORT MODE ipdata_offset:
	 *
	 * Encrypt side (is_encrypt=1):
	 *   Packet has [ETH][IP][UDP][ProtoMeta][IV][Transport+Data]
	 *   ipdata_offset = 14 + 20 + 8 + 2 + 16 = 60
	 *   CAAM encrypts from byte 60 onward (transport header + payload).
	 *
	 * Decrypt side (is_encrypt=0):
	 *   tunnel_decap already stripped [UDP+ProtoMeta+IV] before this call.
	 *   Packet is now [ETH][IP (proto restored)][Encrypted-Transport+Data]
	 *   ipdata_offset = 14 + 20 = 34
	 *   CAAM decrypts from byte 34 onward.
	 */
	if (cparams->is_encrypt) {
		ipdata_offset = sizeof(struct rte_ether_hdr)
		                ...
		                + TRANSPORT_PROTO_META_LEN
		                + IV_IN_PACKET_SIZE;   /* = 60 */
	} else {
		ipdata_offset = sizeof(struct rte_ether_hdr)
		                + sizeof(struct rte_ipv4_hdr);   /* = 34 */
	}
	...
	if (cparams->do_cipher)
	{
		 ...
		 *
		 * Encrypt (is_encrypt=1):
		 *   IV already written into the packet by transport_encap in the
		 *   RX loop.  Read it back from the packet's fixed position so
		 *   CAAM and the wire packet use the same IV.
		 *   Also stash in mbuf headroom for use at dequeue (not needed
		 *   for transport mode since IV is already in the packet, but
		 *   kept for consistency).
		 *
		 * Decrypt (is_encrypt=0):
		 *   pkt_iv is a C stack-local array extracted from the wire
		 *   packet in the RX loop before this call.
		 */
		if (cparams->is_encrypt) {
			generate_per_pkt_iv(iv_ptr);
			/* Write the same IV into the packet at the fixed position */
			char *pkt_base = rte_pktmbuf_mtod(m, char *);
			uint8_t *wire_iv = (uint8_t *)(pkt_base
			                   + sizeof(struct rte_ether_hdr)
			                   + sizeof(struct rte_ipv4_hdr)
			                   + sizeof(struct rte_udp_hdr)
			                   + TRANSPORT_PROTO_META_LEN);
			rte_memcpy(wire_iv, iv_ptr, IV_IN_PACKET_SIZE);
			/* Also stash in headroom for dequeue (not strictly needed
			 * in transport mode but keeps the pattern consistent) */
...
/*
 * transport_encap (called tunnel_encap for interface consistency)
 *
 * TRANSPORT MODE encrypt side — original IP header stays in place.
 *
 * Inserts [UDP 8B][ProtoMeta 2B][IV 16B] between the original IP header
 * and the original transport payload.
 *
 * Before:
 *   [ETH 14B][Original-IP 20B][TCP/UDP/ICMP hdr + Data]
 *
 * After:
 *   [ETH 14B][Original-IP 20B][UDP 8B][ProtoMeta 2B][IV 16B]
 *   [TCP/UDP/ICMP hdr + Data]   ← will be encrypted by CAAM
 *
 * Original IP modifications:
 *   - next_proto_id saved to ProtoMeta, overwritten with IPPROTO_UDP
 *   - total_length increased by TRANSPORT_OVERHEAD bytes
 *   - checksum recomputed
 *
 * IV is generated by l2fwd_simple_crypto_enqueue (called after this),
 * and written into the wire packet at the fixed offset.
 * Here we write zeroes as placeholder; the enqueue function overwrites.
 */
static int
tunnel_encap(struct rte_mbuf *m, uint16_t src_portid __rte_unused,
             const uint8_t *iv_data __rte_unused)
{
    struct rte_ipv4_hdr *ip_hdr;
    struct rte_udp_hdr  *udp_hdr;
    uint8_t             *proto_meta;
    uint16_t             orig_total_len;
    uint8_t              orig_proto;
    uint16_t             transport_len;  /* bytes from transport hdr to end */

    ip_hdr = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(m, char *)
             + sizeof(struct rte_ether_hdr));

    orig_proto     = ip_hdr->next_proto_id;
    orig_total_len = rte_be_to_cpu_16(ip_hdr->total_length);
    transport_len  = (uint16_t)(orig_total_len
                     - (ip_hdr->version_ihl & RTE_IPV4_HDR_IHL_MASK)
                       * RTE_IPV4_IHL_MULTIPLIER);

    /*
     * Insert TRANSPORT_OVERHEAD bytes between IP header and transport payload.
     * Strategy: append space at the tail, memmove the transport data backward,
     * then write the new headers into the gap.
     *
     * This avoids touching the headroom (which is already used for ETH + IP)
     * and is safe for any mbuf with sufficient tailroom.
     */
    if (rte_pktmbuf_append(m, TRANSPORT_OVERHEAD) == NULL)
        return -1;

    char *ip_end = rte_pktmbuf_mtod(m, char *)
                   + sizeof(struct rte_ether_hdr)
                   + sizeof(struct rte_ipv4_hdr);

    /* Shift transport data right to make room */
    memmove(ip_end + TRANSPORT_OVERHEAD, ip_end, transport_len);

    /* Write outer UDP header */
    udp_hdr = (struct rte_udp_hdr *)ip_end;
    udp_hdr->src_port   = rte_cpu_to_be_16(TRANSPORT_UDP_SRC);
    udp_hdr->dst_port   = rte_cpu_to_be_16(TRANSPORT_UDP_DST);
    ...
    /* Write ProtoMeta: 2 bytes storing the original IP protocol */
    proto_meta    = (uint8_t *)ip_end + sizeof(struct rte_udp_hdr);
    proto_meta[0] = orig_proto;
    proto_meta[1] = 0;   /* reserved */

    /* IV placeholder — overwritten by l2fwd_simple_crypto_enqueue */
    memset(proto_meta + TRANSPORT_PROTO_META_LEN, 0, IV_IN_PACKET_SIZE);

    /* Patch original IP header */
    ip_hdr->next_proto_id = IPPROTO_UDP;
    ip_hdr->total_length  = rte_cpu_to_be_16(
                                (uint16_t)(orig_total_len + TRANSPORT_OVERHEAD));
...
/*
 * transport_decap (called tunnel_decap for interface consistency)
 *
 * TRANSPORT MODE decrypt side.
 *
 * Received wire layout:
 *   [ETH][Original-IP][UDP][ProtoMeta 2B][IV 16B][Encrypted-Transport+Data]
 *
 * This function:
 *   1. Extracts the original protocol from ProtoMeta.
 *   2. Extracts the IV from its fixed position — returned in out_iv.
 *   3. Strips [UDP + ProtoMeta + IV] from the mbuf.
 *   4. Restores original IP proto and patches IP total_length + checksum.
 *
 * After return:
 *   [ETH][Original-IP (proto restored)][Encrypted-Transport+Data]
 * ready for CAAM decryption with ipdata_offset = ETH+IP = 34.
 *
 * Wait — in transport mode CAAM still needs to know where encrypted data
 * starts.  After stripping the overhead, the encrypted data starts right
 * after the original IP header, i.e. at offset 34.  So after this function
 * ipdata_offset in l2fwd_simple_crypto_enqueue will point correctly.
 *
 * Actually to keep things simple we strip ALL the overhead here and pass
 * ipdata_offset = ETH+IP = 34 to CAAM.
 */
static int
tunnel_decap(struct rte_mbuf *m)
{
    ...
    /* Verify this is our transport-mode packet */
    if (ip_hdr->next_proto_id != IPPROTO_UDP)
        return -1;

    orig_total_len = rte_be_to_cpu_16(ip_hdr->total_length);

    udp_start  = (uint8_t *)ip_hdr + sizeof(struct rte_ipv4_hdr);
    proto_meta = udp_start + sizeof(struct rte_udp_hdr);
    orig_proto = proto_meta[0];

    /* Shift the encrypted transport data left to close the gap */
    uint16_t ip_hdr_len = (uint16_t)((ip_hdr->version_ihl
                           & RTE_IPV4_HDR_IHL_MASK) * RTE_IPV4_IHL_MULTIPLIER);

    /* Guard: orig_total_len must be large enough to hold ip_hdr + overhead */
    if (orig_total_len < ip_hdr_len + (uint16_t)TRANSPORT_OVERHEAD)
        return -1;   /* malformed / not our packet — drop */

    uint16_t enc_len = (uint16_t)(orig_total_len - ip_hdr_len
                        - (uint16_t)TRANSPORT_OVERHEAD);

    memmove(udp_start, udp_start + TRANSPORT_OVERHEAD, enc_len);

    /* Trim the tail by TRANSPORT_OVERHEAD bytes */
    rte_pktmbuf_trim(m, TRANSPORT_OVERHEAD);

    /* Restore original IP fields */
    ip_hdr->next_proto_id = orig_proto;
    ip_hdr->total_length  = rte_cpu_to_be_16(
                                (uint16_t)(orig_total_len - TRANSPORT_OVERHEAD));
    ip_hdr->hdr_checksum  = 0;
    ip_hdr->hdr_checksum  = rte_ipv4_cksum(ip_hdr);
                     ...
                     const uint8_t *iv __rte_unused)
{
    ...
    if (portid == 0) {
        /*
         * Encrypt side:
         * tunnel_encap() inserted [UDP][ProtoMeta][IV_placeholder] before
         * the transport header in the RX loop BEFORE enqueue.
         * IV was written into the packet by l2fwd_simple_crypto_enqueue.
         * CAAM has now encrypted the transport header + data.
         * Nothing more to do — just forward.
         *
         * Wire packet is already:
         *   [ETH][IP (proto=UDP, len+=overhead)][UDP][ProtoMeta][IV]
         *   [Encrypted-Transport+Data][AES-CBC padding]
         */
        /* (no further header manipulation needed) */

    } else if (portid == 1) {
        /*
         * Decrypt side:
         * tunnel_decap() stripped [UDP+ProtoMeta+IV], restored IP proto
         * and total_length BEFORE enqueue (in the RX loop).
         * CAAM has now decrypted the transport header + data.
         *
         * Trim auth digest then AES-CBC padding using restored IP total_length.
         */
        if (options->auth_xform.auth.digest_length > 0)
            rte_pktmbuf_trim(m, options->auth_xform.auth.digest_length);

        ip_hdr = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(m, char *) + eth_len);
        pad_len = m->pkt_len
                  - eth_len
		...
		for (i = 0; i < qconf->nb_rx_ports; i++) {
			...
			if (nb_rx) {
				...
				for (j = 0; j < nb_rx; j++) {
					...
					/* ARP: forward without crypto, free pre-allocated op */
					...
					if (portid == 0) {
						/*
						 * TRANSPORT MODE ENCRYPT path:
						 *
						 * Original packet:
						 *   [ETH][IP (proto=TCP/UDP/etc)][Transport hdr+Data]
						 *
						 * Step 1: tunnel_encap inserts [UDP][ProtoMeta][IV_placeholder]
						 *         between IP header and transport header using memmove.
						 *         Patches IP proto → UDP, increases IP total_length.
						 *
						 * After encap:
						 *   [ETH][IP (proto=UDP, len+=26)][UDP][ProtoMeta 2B][IV 16B]
						 *   [Transport hdr + Data]
						 *
						 * Step 2: l2fwd_simple_crypto_enqueue generates IV,
						 *         writes it into the packet at offset 44,
						 *         tells CAAM to encrypt from ipdata_offset=60 onward.
						 */
						if (tunnel_encap(m, portid, NULL) < 0) {
							rte_pktmbuf_free(m);
							rte_crypto_op_free(ops_burst[j]);
							continue;
						}
						...
						/*
						 * TRANSPORT MODE DECRYPT path:
						 *
						 * Received wire layout:
						 *   [ETH][IP (proto=UDP)][UDP 8B][ProtoMeta 2B][IV 16B]
						 *   [Encrypted-Transport+Data]
						 *
						 * CRITICAL ORDER: extract IV FIRST before tunnel_decap
						 * because tunnel_decap uses memmove to shift the encrypted
						 * data left, which overwrites the IV position.
						 *
						 * Step 1: Extract IV into stack-local array from its fixed
						 *         position: ETH(14) + IP(20) + UDP(8) + Meta(2) = 44.
						 *
						 * Step 2: tunnel_decap strips [UDP+ProtoMeta+IV] via memmove,
						 *         restores original IP proto and total_length.
						 *         After decap: [ETH][IP (proto restored)][Encrypted]
						 *
						 * Step 3: Enqueue to CAAM. ipdata_offset=34 (ETH+IP) so
						 *         CAAM decrypts from right after the IP header.
						 */
						uint8_t pkt_iv[IV_IN_PACKET_SIZE];
						{
							char *base = rte_pktmbuf_mtod(m, char *);
							struct rte_ipv4_hdr *pip =
								(struct rte_ipv4_hdr *)(base
								+ sizeof(struct rte_ether_hdr));
							uint16_t ip_hl = (uint16_t)(
								(pip->version_ihl & RTE_IPV4_HDR_IHL_MASK)
								* RTE_IPV4_IHL_MULTIPLIER);
							/*
							 * IV position in received packet:
							 *   ETH(14) + IP(ip_hl) + UDP(8) + Meta(2) = 14+ip_hl+10
							 */
							uint8_t *iv_pos = (uint8_t *)(base
								+ sizeof(struct rte_ether_hdr)
								+ ip_hl
								+ sizeof(struct rte_udp_hdr)
								+ TRANSPORT_PROTO_META_LEN);
							rte_memcpy(pkt_iv, iv_pos, IV_IN_PACKET_SIZE);
						}
			...
			do
			{
				...
				for (j = 0; j < nb_rx; j++)
				{
					...
					/*
					 * Both encrypt and decrypt: l2fwd_simple_forward just
					 * trims padding on the decrypt side and forwards.
					 * iv parameter unused in transport forward (NULL ok).
					 */
					l2fwd_simple_forward(m, portid, options, NULL);
				}
