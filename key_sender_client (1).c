 ...
 * Run on RHEL box.
 *
 * Identical structure to key_sender_server.c -- this process is also the
 * AF_UNIX SERVER. It creates /run/l2fwd/keyd.sock on the RHEL box and
 * waits for the local DPDK app to connect to it.
 *
 * Sequence:
 *   1. Create /run/l2fwd/keyd.sock and listen
 *   2. Wait for DPDK app to connect
 *   3. Call get_client_tls_keys() -- full PQC TLS handshake, blocks until done
 *   4. Write keys[0] first 16 bytes to DPDK via socket
 *   5. Sleep ROTATE_INTERVAL_SEC seconds
 *   6. Repeat from step 3
 *
 * Compile:
 *   gcc src/key_sender_client.c lib/call_client_key.o \
 ...
 *       -o bin/key_sender_client
 *
 * Run BEFORE the DPDK app:
 *   mkdir -p /run/l2fwd
 *   LD_LIBRARY_PATH=/usr/lib/jvm/jdk-21.0.7-oracle-x64/lib/server \
 *       bin/key_sender_client
 */
...
/* ── function prototype ──────────────────────────────────────────────────── */
extern jbyte **get_client_tls_keys(const char *url,
                                    const char *ip,
                                    int port,
                                    int key_lengths[2]);
...
int main(void)
{
    fprintf(stderr, "[KEY] key_sender_client starting\n");
    fprintf(stderr, "[KEY] url=%s ip=%s port=%d rotate=%ds\n",
            URL, IP, PORT, ROTATE_INTERVAL_SEC);
    ...
    while (1) {
        ...
        while (1) {
            ...
            fprintf(stderr, "[KEY] calling get_client_tls_keys...\n");
            jbyte **keys = get_client_tls_keys(URL, IP, PORT, key_lengths);
