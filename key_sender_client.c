 ...
 * key_sender_client.c
 *
 * Rewritten from test_client_key_main.c.
 *
 * Original behaviour: call get_client_tls_keys() ONCE, print the keys,
 * exit.
 *
 * New behaviour: call get_client_tls_keys() in an infinite loop. Your
 ...
 *
 * Usage:
 *  Run this on the RHEL / client box.
 *  Compile and link exactly as the original test_client_key_main.c was.
 *
 ...
 *   mkdir -p /run/l2fwd
 *   LD_LIBRARY_PATH=/opt/jdk-21.0.7/lib/server bin/key_sender_client
 */
...
extern jbyte **get_client_tls_keys(const char *url, const char *ip, int port, int key_lengths[2]);
...
int main(void)
{
    ...
    fprintf(stderr, "[KEY] url=%s ip=%s port=%d\n", URL, IP, PORT);
